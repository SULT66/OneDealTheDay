...
async function loadProducts(c) {
  ...
  if (!c.rainforestApiKey)
    throw new Error("RAINFOREST_API_KEY is missing");
...
exports.refreshProducts = async c => {
  ...
  const runId = Number(
    db.prepare(`
      ...
      (provider,started_at,status,message)
      VALUES(?,?,'running','')
    `).run(c.provider, started).lastInsertRowid
  );
  ...
  try {
    ...
    if (!Array.isArray(found) || found.length < 10 || top.length < 10) {
      throw new Error(
        `Refresh returned insufficient products (${top.length}/10)`
      );
    }
...
`);
      ...
      for (const product of top) {
        ...
        const safe = {
          ...
          external_id: textValue(product.external_id),
          ...
          model_number: textValue(product.model_number),
          ...
          review_count: Math.round(
            numberValue(product.review_count,0)
          ),

          current_price:
            product.current_price == null
              ? null
              : numberValue(product.current_price,null),

          original_price:
            product.original_price == null
              ? null
              : numberValue(product.original_price,null),
      ...
      const ids = top.map(p => p.external_id);
...
published_count=?,
status='success',
message='Amazon and Walmart products published'
WHERE id=?
`).run(
