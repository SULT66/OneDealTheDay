...
const money = (value, currency = "USD", locale = "en-US") => { const n = Number(value); if (!Number.isFinite(n)) return "Check latest price"; try { return new Intl.NumberFormat(locale, { style: "currency", currency: String(currency || "USD").toUpperCase() }).format(n); } catch { return `$${n.toFixed(2)}`; } };
...
app.get("/deal/:slug", (req, res) => {
  ...
  const ratingSummary = Number(p.rating)>0&&Number(p.review_count)>0
    ...
    : "";
  const body = `<main class="product-page"><nav class="breadcrumb"><a href="${marketPath(p.market)}">Home</a><span>›</span><a href="${catPath(category,p.market)}">${esc(displayCategory)}</a>${p.brand?`<span>›</span><a href="${brandPath(p.brand,p.market)}">${esc(p.brand)}</a>`:""}<span>›</span><span>${esc(title)}</span></nav><article class="product-detail"><div class="product-detail-media"><img src="${esc(p.image_url)}" alt="${esc(title)}" decoding="async"></div><div class="product-detail-content">${brandBlock}<h1>${esc(title)}</h1><div class="product-score"><strong>${Math.round(Number(p.score)||0)}/100</strong><span>OneDailyDrop ${esc(t(req.language,"product.score"))}</span></div><p class="product-lead">${esc(description)}</p><section class="editorial-box"><h2>Why we picked it</h2><p>${esc(whyPicked(display, req.language))}</p></section><div class="detail-grid"><section><h3>Current price</h3><p>${money(p.current_price,p.currency,pageLocale)}</p></section>${ratingSummary}<section><h3>30-day tracked low</h3><p>${historyLabel(intelligence.day30)}</p></section><section><h3>90-day tracked low</h3><p>${historyLabel(intelligence.day90)}</p></section><section><h3>All-time tracked low</h3><p>${historyLabel(intelligence.allTime)}</p></section></div><section class="editorial-box"><h2>Price history</h2>${chartSvg(history,req.language,p.market)}<p>${esc(t(req.language,"page.historySummary",{observations:history.length,days:intelligence.allTime.distinctDays}))}</p></section><div class="product-price-box"><span class="product-price">${money(p.current_price,p.currency,pageLocale)}</span>${p.original_price?`<span class="old">${money(p.original_price,p.currency,pageLocale)}</span>`:""}<small>Final price is confirmed on the retailer website.</small></div><a class="featured-button" href="${marketPath(p.market, `/go/${p.id}`)}" rel="nofollow sponsored">See deal on ${esc(store)} →</a></div></article>${relatedBlock}</main>`;
...
app.get("/api/status", (req,res) => res.json({provider:c.provider,products:db.prepare(`SELECT COUNT(*) n FROM products WHERE status='published' AND ${sourceSql()}`).get().n,brands:db.prepare(`SELECT COUNT(DISTINCT brand_slug) n FROM products WHERE status='published' AND ${sourceSql()} AND brand_slug<>''`).get().n,clicks:db.prepare(`SELECT COUNT(*) n FROM clicks c JOIN products p ON p.id=c.product_id WHERE ${sourceSql("p")}`).get().n,priceObservations:db.prepare(`SELECT COUNT(*) n FROM price_history h JOIN products p ON p.id=h.product_id WHERE ${sourceSql("p")}`).get().n,lastRun:db.prepare("SELECT * FROM refresh_runs ORDER BY id DESC LIMIT 1").get()}));
