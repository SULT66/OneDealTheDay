...
const isManualProduct = product => String(product?.source || "").toLowerCase() === "amazon-manual";
...
app.get("/deal/:slug", (req, res) => {
  ...
  const checkedLabel = checkedAt && !Number.isNaN(new Date(checkedAt).getTime())
    ...
    : "Recently";
  const manualProduct = isManualProduct(p);
  const retailerDetails = liveOffer ? `<div class="detail-grid retailer-detail-grid" aria-label="Retailer details"><section><h3>Retailer</h3><p>${esc(store)}</p></section><section><h3>Sold by</h3><p>${esc(manualProduct ? "Confirm on Amazon" : clean(p.seller_name) || store)}</p></section><section><h3>Delivery</h3><p>${esc(clean(p.shipping_summary) || "Confirm at retailer")}</p></section><section><h3>Returns</h3><p>${esc(clean(p.return_summary) || "See retailer policy")}</p></section><section><h3>Availability</h3><p>${esc(clean(p.availability) || "Confirm at retailer")}</p></section>${manualProduct ? "" : `<section><h3>Price checked</h3><p>${esc(checkedLabel)}</p></section>`}</div>` : "";
  ...
  const ratingSummary = Number(p.rating)>0&&Number(p.review_count)>0
    ...
    : "";
  const scoreBlock = manualProduct ? "" : `<div class="product-score"><strong>${Math.round(Number(p.score)||0)}/100</strong><span>OneDailyDrop ${esc(t(req.language,"product.score"))}</span></div>`;
  const priceDetails = manualProduct
    ? `<div class="detail-grid"><section><h3>Current price</h3><p>Check current price on Amazon</p></section></div>`
    : `<div class="detail-grid"><section><h3>Current price</h3><p>${money(p.current_price,p.currency,pageLocale)}</p></section>${ratingSummary}<section><h3>30-day tracked low</h3><p>${historyLabel(intelligence.day30)}</p></section><section><h3>90-day tracked low</h3><p>${historyLabel(intelligence.day90)}</p></section><section><h3>All-time tracked low</h3><p>${historyLabel(intelligence.allTime)}</p></section></div><section class="editorial-box"><h2>Price history</h2>${chartSvg(history,req.language,p.market)}<p>${esc(t(req.language,"page.historySummary",{observations:history.length,days:intelligence.allTime.distinctDays}))}</p></section>`;
  const body = `<main class="product-page"><nav class="breadcrumb"><a href="${marketPath(p.market)}">Home</a><span>›</span><a href="${catPath(category,p.market)}">${esc(displayCategory)}</a>${p.brand?`<span>›</span><a href="${brandPath(p.brand,p.market)}">${esc(p.brand)}</a>`:""}<span>›</span><span>${esc(title)}</span></nav><article class="product-detail"><div class="product-detail-media"><img src="${esc(p.image_url)}" alt="${esc(title)}" decoding="async"></div><div class="product-detail-content">${brandBlock}<h1>${esc(title)}</h1>${scoreBlock}<p class="product-lead">${esc(description)}</p><section class="editorial-box"><h2>Why we picked it</h2><p>${esc(whyPicked(display, req.language))}</p></section>${priceDetails}<div class="product-price-box"><span class="product-price">${manualProduct ? "Check current price on Amazon" : money(p.current_price,p.currency,pageLocale)}</span>${p.original_price?`<span class="old">${money(p.original_price,p.currency,pageLocale)}</span>`:""}<small>Final price is confirmed on the retailer website.</small></div><a class="featured-button" href="${marketPath(p.market, `/go/${p.id}`)}" rel="nofollow sponsored">See deal on ${esc(store)} →</a></div></article>${relatedBlock}</main>`;
