...
const shell = (title, description, canonical, body, schema = null, image = "", robots = "", code = "us", alternateCodes = marketCodes, req = null) => {
  ...
  const versionedHtml = html
    ...
    .replace("</head>", '<link rel="stylesheet" href="/i18n.css?v=20260808-assistant"><link rel="stylesheet" href="/shopping-assistant.css?v=20260811-primary-product-relevance"></head>')
    .replace("</body>", `${shoppingAssistantPanel(code, language)}<script src="/shopping-assistant.js?v=20260811-primary-product-relevance"></script></body>`);
...
const sameProductOffers = product => {
  ...
  const offers = db.prepare(`SELECT * FROM products WHERE market=? AND status='published' AND ${sourceSql()} AND product_key=? ORDER BY current_price ASC,score DESC`).all(product.market, productKey);
  return offers.length ? offers : [product];
};
