...
const whyPicked = product => { const points = []; if (Number(product.rating) >= 4.5) points.push(`strong ${Number(product.rating).toFixed(1)}-star rating`); if (Number(product.review_count) >= 1000) points.push(`${Number(product.review_count).toLocaleString()}+ reviews`); if (Number(product.score) >= 80) points.push(`high Our deal score of ${Math.round(Number(product.score))}`); if (discountPercent(product) > 0) points.push(`${discountPercent(product)}% verified price reduction`); return points.length ? `We selected this product for its ${points.join(", ")}.` : "We selected this product after reviewing its price, customer feedback, availability and overall value."; };
...
const sharedHeader = () => `<header class="site-header"><div class="header-top"><a class="brand" href="/"><span class="brand-mark">D</span><span class="brand-copy"><strong>OneDailyDrop</strong><small>A better place to start before you buy.</small></span></a><form class="header-search" action="/search"><span aria-hidden="true">⌕</span><input name="q" type="search" placeholder="Search deals" aria-label="Search deals"></form><a class="header-subscribe" href="/#subscribe">Get Daily Drops</a><button id="themeToggle" class="theme-button" type="button" aria-label="Switch to dark mode" title="Dark mode"><span class="theme-button-icon" aria-hidden="true">☾</span><span class="theme-button-label">Dark</span></button></div><nav class="main-nav" aria-label="Primary navigation"><a href="/">Today</a><div class="category-menu"><button type="button" aria-expanded="false">Categories <span>⌄</span></button><div class="mega-menu" hidden>${navCategories().map(category => `<a href="${catPath(category)}">${esc(category)}</a>`).join("")}</div></div><a href="/#trending">Trending</a><a href="/archive">Yesterday's Drops</a><a href="/about">About</a><span data-account-nav></span></nav></header>`;
const sharedFooter = () => `<footer><div class="footer-brand"><b>OneDailyDrop</b><p>A better place to start before you buy.</p><div class="footer-links"><a href="/about">About</a><a href="/contact">Contact</a><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="/affiliate-disclosure">Affiliate Disclosure</a><a href="/editorial-policy">Editorial Policy</a></div></div><p class="disclosure">We choose products independently. If you buy through one of our links, we may earn a commission at no extra cost to you.</p></footer>`;
...
const archiveCard = product => `<article class="archive-card"><a class="archive-card-media" href="${dealPath(product)}"><img src="${esc(product.image_url)}" alt="${esc(shortTitle(product.title))}"></a><div class="archive-card-content">${product.category ? `<a class="eyebrow" href="${catPath(product.category)}">${esc(product.category)}</a>` : ""}<h2><a href="${dealPath(product)}">${esc(shortTitle(product.title))}</a></h2><p class="description">${esc(whyPicked(product))}</p><span class="price">${money(product.current_price, product.currency)}</span><a class="button" href="${dealPath(product)}">View details</a></div></article>`;
...
app.get("/deal/:slug", (req, res) => {
  ...
  const body = `<main class="product-page"><nav class="breadcrumb"><a href="/">Home</a><span>›</span><a href="${catPath(category)}">${esc(category)}</a>${p.brand?`<span>›</span><a href="${brandPath(p.brand)}">${esc(p.brand)}</a>`:""}<span>›</span><span>${esc(title)}</span></nav><article class="product-detail"><div class="product-detail-media"><img src="${esc(p.image_url)}" alt="${esc(title)}"></div><div class="product-detail-content">${brandBlock}<h1>${esc(title)}</h1><div class="product-score"><strong>${Math.round(Number(p.score)||0)}/100</strong><span>Our deal score</span></div><p class="product-lead">${esc(description)}</p><section class="editorial-box"><h2>Why it made the list</h2><p>${esc(whyPicked(p))}</p></section><div class="detail-grid"><section><h3>Current price</h3><p>${money(p.current_price,p.currency)}</p></section><section><h3>30-day low</h3><p>${money(low30,p.currency)}</p></section><section><h3>90-day low</h3><p>${money(low90,p.currency)}</p></section><section><h3>All-time tracked low</h3><p>${money(allLow,p.currency)}</p></section></div><section class="editorial-box"><h2>Price history</h2>${chartSvg(history)}<p>${history.length} tracked price observation${history.length===1?"":"s"}.</p></section><div class="product-price-box"><span class="product-price">${money(p.current_price,p.currency)}</span>${p.original_price?`<span class="old">${money(p.original_price,p.currency)}</span>`:""}<small>Check the retailer's page for the final price.</small></div><a class="featured-button" href="/go/${p.id}" rel="nofollow sponsored">Check price at ${esc(store)}</a></div></article>${relatedBlock}</main>`;
...
app.get("/archive", (req, res) => {
  ...
  const dates = products.map((product, index) => {
    ...
    return `<article class="archive-row"><time datetime="${date.toISOString().slice(0,10)}">${label}</time>${archiveCard(product)}</article>`;
  }).join("");
  const body = `<main><section class="deals-section"><div class="section-heading"><div><p class="eyebrow">ARCHIVE</p><h1>Yesterday's Drops</h1><p class="description">See the deals we featured on previous days.</p></div></div><div class="archive-list">${dates || '<div class="empty-state">There are no past drops yet.</div>'}</div></section></main>`;
  res.send(shell("Yesterday's Drops | OneDailyDrop", "Browse earlier OneDailyDrop picks by date.", `${SITE}/archive`, body, null));
});
...
app.get("/brand/:slug", (req, res) => {
  ...
  const description = `See ${products.length} ${brand} products currently listed on OneDailyDrop.`;
...
app.use((req, res) => {
  ...
  const body = `<main class="not-found"><p class="eyebrow">404 ERROR</p><h1>We could not find that page.</h1><p>It may have moved or the address may be incorrect.</p><div class="hero-actions"><a class="primary-cta" href="/">Back to Today's Drop</a><a class="secondary-cta" href="/search">Search deals</a></div></main>`;
